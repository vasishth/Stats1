...
2. Use the `sample` function to draw 50 random numbers from `vec3` and save the result in a new vector `data`. You can pick yourself if you want to draw with or without replacement!
_Tip:_ Use the `?` function and Google to find out how `sample` works!
An example use: 
```{r eval = FALSE}
candies <- c("licorice", "fudge", "gummy bears", "chocolate")
data <- sample(candies, 20, replace = FALSE)
```

3. Calculate the mean of `data`.

4. Calculate the standard deviation of `data`.